...
var RethinkDBFSBucket = {
  ...
  initBucket () {
    return co(function *() {
      ...
      if (!includes(fileIndexList, 'status_path_finishedat')) {
        promises.push(this.r.table(this.conf.fileTable).indexCreate('status_path_finishedat', [this.r.row('status'), function (x) { return x('filename').split('/').slice(1, -1) }, this.r.row('finishedAt')]))
      }
