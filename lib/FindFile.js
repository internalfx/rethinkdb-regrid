...
var FindFile = function (bucket, pattern, options) {
...
util.inherits(FindFile, Readable)

FindFile.prototype._read = function () {
  co(function *() {
    var file

    if (!this.cursor) {
      let query = this.r.table(this.conf.fileTable).filter(this.r.row('filename').match(this.pattern))
