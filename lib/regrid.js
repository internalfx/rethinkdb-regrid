...
let ReGrid = function (connOpts, bucketOptions) {
  ...
  let fetchFileByName = Promise.coroutine(function *(filename, revision = -1) {
    ...
    if (file == null) {
      console.log(`FILE NAME CACHE MISS: ${fileKey}`)
      let revSteps
      ...
      console.log(`FILE NAME CACHE HIT: ${fileKey}`)
    }
