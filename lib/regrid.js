...
let ReGrid = function (connOpts, bucketOptions) {
  ...
  let readFile = async function (spec = {}) {
    ...
    let stream = createReadStream({id: file.id})
  ...
  let createReadStream = function (spec = {}) {
    ...
    stream._read = Promise.coroutine(function *() {
      ...
      if (file == null) {
        file = yield fetchFileById(spec.id)
  ...
  let downloadFilename = function (filename, options = {}) {
    ...
    stream._read = async function () {
      ...
      if (chunk) {
        if (chunk.num !== num) {
          process.nextTick(function () {
            stream.emit('error', new Error('Chunk number mismatch: File is likely corrupted!'))
          })
          return
        }
        if (verifyHash) { hash.update(chunk.data) }
    ...
    if (options.promise) {
      return rgUtils.readStreamPromise(stream)
    }
