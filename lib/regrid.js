...
let ReGrid = function (connOpts, bucketOpts) {
  ...
  let upload = function (filename, options = {}) {
    ...
    let chunkSizeBytes = this.options.chunkSizeBytes || this.conf.chunkSizeBytes
    let num = 0
    ...
    stream._read = Promise.coroutine(function *() {
      ...
      if (!this.file) {
        ...
        if (endNum != null) {
          ...
          cursor = yield r.table(chunkTable).between([file.id, startNum], [file.id, this.r.maxval], {index: 'chunk_ix', rightBound: 'closed'}).orderBy({index: 'chunk_ix'}).run({cursor: true})
        }
      }
